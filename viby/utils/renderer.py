...
import time
import threading
from typing import Iterator, Optional, Dict, Any, List, Tuple, Callable
...
class MarkdownStreamRenderer:
    ...
    def _should_render(self) -> bool:
        ...
        if time_passed >= self.config.throttle_ms or len(self.buffer) >= self.config.buffer_size:
        ...
        with Live(auto_refresh=False) as live:
            ...
            while i < len(text):
                ...
                if i % 3 == 0 or i == len(text):  # 为提高性能，每3个字符刷新一次
                    ...
                    live.update(Markdown(rendered_text + cursor, justify="left"), refresh=True)
    ...
    def render_stream(self, text_stream: Iterator[str], return_full: bool = True) -> Optional[str]:
        ...
        try:
            ...
            self.console.print(f"[bold red]{get_text('RENDERER', 'render_error', str(e))}[/bold red]")
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rendering_efficiency": 1 - (self.total_render_time / max(0.001, total_time))
        }
...
def render_markdown_stream_optimized(text_stream: Iterator[str], 
                                    config: Optional[RenderConfig] = None,
                                    return_full: bool = True) -> Optional[str]:
    """
    优化版的流式Markdown渲染函数
...
# 便于直接导入的工厂函数
def create_renderer(typing_effect: bool = False,
                   typing_speed: float = 0.01,
                   smooth_scroll: bool = True,
                   throttle_ms: int = 50,
                   buffer_size: int = 10,
                   ...
                   enable_animations: bool = True,
                   code_block_instant: bool = True,
                   theme: Dict[str, Any] = None) -> MarkdownStreamRenderer:
    """
    创建自定义配置的渲染器
