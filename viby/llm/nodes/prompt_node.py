...
class PromptNode(Node):
    ...
    def exec(self, inputs):
        ...
        if config.enable_mcp:
            all_tools, tool_servers = self._fetch_mcp_tools(config, all_tools, tool_servers, server_name)
    ...
    def exec_fallback(self, prep_res, exc):
        """处理执行过程中的错误"""
        user_input = prep_res[0] if isinstance(prep_res, tuple) and len(prep_res) > 0 else ""
