...
class ModelManager:
    ...
    def _prepare_messages(self, messages, model_config) -> Tuple[List[Dict], Optional[str]]:
        """准备消息并处理用户输入"""
        # 重置token跟踪器
        ...
        if messages:
            ...
            if last_user_message:
                ...
                if last_user_message is not self.last_user_message_ref:
                    ...
                    # 应用消息压缩
                    messages, _ = self.compaction_manager.compact_messages(messages, model_config)
    ...
    def _update_history(self, full_response, user_input=None):
        ...
        elif (self.interaction_recorded and 
              self.last_interaction_id and 
