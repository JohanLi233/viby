...
class TestToolRetrieval(unittest.TestCase):
    ...
    def test_tool_embedding_manager_initialization(self):
        ...
        with (
            ...
            patch("viby.tools.embedding_utils.logger") as mock_logger,
            patch("viby.locale.get_text") as mock_get_text,
            patch("viby.tools.embedding_utils.Path.mkdir"),
        ):
    ...
    def test_collect_mcp_tools(self):
        ...
        with (
            ...
            patch("viby.mcp.list_tools") as mock_list_tools,
        ):
            # 模拟MCP启用
            mock_config_instance = MagicMock()
            ...
            # 模拟list_tools返回值
            mock_list_tools.return_value = {"server1": [mock_tool]}
        ...
        with (
            patch("viby.config.Config") as mock_config_class,
            patch("viby.mcp.list_tools") as mock_list_tools,
            patch("viby.tools.tool_retrieval.logger") as mock_logger,
        ):
            # 模拟MCP启用
            mock_config_instance = MagicMock()
            mock_config_instance.enable_mcp = True
    ...
    def test_search_tools(self, mock_get_manager):
        ...
        # 执行搜索 - 使用patch来模拟远程服务不可用，确保测试的是本地模式
        with (
            patch("viby.tools.tool_retrieval.logger") as mock_logger,
            patch("viby.tools.tool_retrieval.is_server_running", return_value=False),
        ):
            # 设置mock_logger的level属性为整数，避免TypeError
            mock_logger.level = 0
    ...
    def test_execute_tool_retrieval(self, mock_search):
        ...
        with (
            patch(
                "viby.tools.tool_retrieval.search_similar_tools",
        ...
        # 测试正常情况 - 重新设置search_similar_tools以返回预期结果
        mock_result = [{"name": "test_tool", "score": 0.95}]

        # 重置之前的模拟
        mock_search.reset_mock()

        # 直接模拟search_similar_tools函数
        with patch(
            "viby.tools.tool_retrieval.search_similar_tools", return_value=mock_result
        ):
    ...
    def test_execute_update_embeddings(self, mock_get_manager, mock_collect_tools):
        ...
        with (
            patch("viby.tools.tool_retrieval.logger") as mock_logger,
            patch("viby.tools.embedding_server.is_server_running", return_value=False),
        ):
            # 设置mock_logger的level属性为整数，避免TypeError
            mock_logger.level = 0
